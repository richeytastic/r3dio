...
struct ModelResource
{
    ...
    ModelResource( const Mesh* model, bool media9, int matID) : _model(model), _media9(media9)
    {
...
bool IDTFExporter::doSave( const Mesh& inmodel, const std::string& filename)
{
    ...
    for ( int mid : mids)
    {
        ...
        if ( tx.empty())
        {
            ...
            eoss << "[ERROR] r3dio::IDTFExporter::doSave: Material " << mid << " has no texture!";
            setErr(eoss.str());
            return false;
        }   // end else
