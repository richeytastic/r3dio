...
// public
LaTeXU3DInserter::Ptr LaTeXU3DInserter::create( const Mesh& model,
...
bool LaTeXU3DInserter::setModel( const std::string& u3dfile)
{
    ...
    if ( boost::algorithm::to_lower_copy( Path(u3dfile).extension().string()) != ".u3d")
        std::cerr << "[ERROR] r3dio::LaTeXU3DInserter::setModel: " << u3dfile << " must have .u3d extension!" << std::endl;
    else
    {
        _u3dfile = u3dfile;
        setokay = true;
...
bool LaTeXU3DInserter::setModel( const Mesh& model, const std::string& sdir)
{
...
std::ostream& r3dio::operator<<( std::ostream& os, const LaTeXU3DInserter& params)
{
    ...
    Vec3f c2c = cpos - coo;
    c2c.normalize(); // Shouldn't need normalizing.
